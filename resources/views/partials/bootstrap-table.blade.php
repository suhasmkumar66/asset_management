...
<script nonce="{{ csrf_token() }}">
    ...
    $(function () {
        ...
        $('.Suhas-table').bootstrapTable('destroy').bootstrapTable({
            ...
            pageSize: {{  (($SuhasSettings->per_page!='') && ($SuhasSettings->per_page > 0)) ? $SuhasSettings->per_page : 20 }},
    ...
    function dateRowCheckStyle(value) {
        if ((value.days_to_next_audit) && (value.days_to_next_audit < {{ $SuhasSettings->audit_warning_days ?: 0 }})) {
    ...
    $('.Suhas-table').on('check.bs.table', function () {
        $('#bulkEdit').removeAttr('disabled');
    });

    $('.Suhas-table').on('check-all.bs.table', function () {
        $('#bulkEdit').removeAttr('disabled');
    });

    $('.Suhas-table').on('uncheck.bs.table', function () {
        if ($('.Suhas-table').bootstrapTable('getSelections').length == 0) {
            $('#bulkEdit').attr('disabled', 'disabled');
        }
    });

    $('.Suhas-table').on('uncheck-all.bs.table', function (e, row) {
        $('#bulkEdit').attr('disabled', 'disabled');
    });
    ...
    function ticketsstatusValue(value){
        if(value == 'open'){
          return '<span class="label label-primary" style="padding: 6px 11px;font-size: 11px;font-weight: 500;line-height: 2.7;">'+value+'</span>';
        }
        else if(value == 'assigned'){
          return '<span class="label label-default" style="padding: 6px 11px;font-size: 11px;font-weight: 500;line-height: 2.7;">'+value+'</span>';
        }
        else if(value == 'inprogress'){
          return '<span class="label label-warning" style="padding: 6px 11px;font-size: 11px;font-weight: 500;line-height: 2.7;">'+value+'</span>';
        }
        else if(value == 'rejected'){
          return '<span class="label label-danger" style="padding: 6px 11px;font-size: 11px;font-weight: 500;line-height: 2.7;">'+value+'</span>';
        }
        else if(value == 'closed'){
          return '<span class="label label-info" style="padding: 6px 11px;font-size: 11px;font-weight: 500;line-height: 2.7;">'+value+'</span>';
        }
        else if(value == 'acknowledged'){
          return '<span class="label label-success" style="padding: 6px 11px;font-size: 11px;font-weight: 500;line-height: 2.7;">'+value+'</span>';
        }
        else{
          return '<span class="label label-default" style="padding: 6px 11px;font-size: 11px;font-weight: 500;line-height: 2.7;">'+value+'</span>';
        }
    ...
    function genericAssignFormatter(destination){
    	  return function (value,row) {
              if (value) {
                  return '<a href="{{ url('/') }}/' + destination + '/' + row.id +  '/' + 'assign' + '"> ' + value + '</a>';
              }
          };
    }
	...
	function genericAssignActionFormatter(destination){
		 return function (value,row) {

			 if(value == 'assigned' || value == 'inprogress' || value == 'closed'){
				 assign_status = 'Assigned';
			 }
			 else if(value == 'acknowledged'){
				 assign_status = 'Re-open';
			 }
			 else{
				 assign_status = 'Assign';
			 }
             return '<a href="{{ url('/') }}/' + destination + '/' + row.id + '/assign" class="btn btn-sm bg-maroon" data-tooltip="true" title="Assign this Ticket">'+assign_status+'</a>';

		 }
	}

	function genericSelfActionFormatter(destination){
		 return function (value,row) {
             return '<a href="{{ url('/') }}/' + destination + '/' + row.id + '/selfassign" class="btn btn-sm bg-green" data-tooltip="true" title="Self Assign To you">Self Assign</a>';

		 }
	}

	function genericLogsActionFormatter(destination){
		 return function (value,row) {
            return '<a href="{{ url('/') }}/' + destination + '/' + row.id + '/logs" class="btn btn-sm bg-blue" data-tooltip="true" title="Logs of this Ticket">Logs</a>';

		 }
	}

	function genericAgentActionFormatterexample(status,row){
		var actions = '<nobr>';

        // Add some overrides for any funny urls we have
    var dest = 'tickets';
	if(status == 'open' || status == 'closed' || status == 'acknowledged'){
    ...
    actions += '<a href="{{ url('/') }}/' + dest + '/' + row.id + '/accept" class="btn btn-sm btn-success '+disabled+'" data-tooltip="true" title="Accept Ticket"><i class="fa fa-check"></i></a>&nbsp;';

    actions += '<a href="{{ url('/') }}/' + dest + '/' + row.id + '/transfer" class="btn btn-sm btn-warning '+disabled+'" data-tooltip="true" title="Transfer Ticket"><i class="fa fa-exchange"></i></a>&nbsp;';

    actions += '<a href="{{ url('/') }}/' + dest + '/' + row.id + '/reject" class="btn btn-sm btn-danger '+disabled+'" data-tooltip="true" title="Reject Ticket"><i class="fa fa-ban"></i></a>&nbsp;';

    actions += '<a href="{{ url('/') }}/' + dest + '/' + row.id + '/close" class="btn btn-sm btn-primary '+disabled+'" data-tooltip="true" title="Close Ticket"><i class="fa fa-close"></i></a>&nbsp;';

    actions +='</nobr>';
    return actions;
	}
	function genericAgentActionFormatter(destination){
		return function (value,row) {
		var actions = '<nobr>';
        ...
        actions += '<a href="{{ url('/') }}/' + dest + '/' + row.id + '/accept" class="btn btn-sm btn-success" data-tooltip="true" title="Accept Ticket"><i class="fa fa-check"></i></a>&nbsp;';

        actions += '<a href="{{ url('/') }}/' + dest + '/' + row.id + '/transfer" class="btn btn-sm btn-warning" data-tooltip="true" title="Transfer Ticket"><i class="fa fa-exchange"></i></a>&nbsp;';

        actions += '<a href="{{ url('/') }}/' + dest + '/' + row.id + '/reject" class="btn btn-sm btn-danger" data-tooltip="true" title="Reject Ticket"><i class="fa fa-ban"></i></a>&nbsp;';

        actions += '<a href="{{ url('/') }}/' + dest + '/' + row.id + '/close" class="btn btn-sm btn-primary" data-tooltip="true" title="Close Ticket"><i class="fa fa-close"></i></a>&nbsp;';
	...
	function AckActionFormatterexample(status,row){
		var actions = '<nobr>';
		  var dest = 'tickets';
			if(status == 'closed'){
				var disabled = '';
				...
				actions += '<a href="{{ url('/') }}/' + dest + '/' + row.id + '/acknowledge" class="btn btn-sm btn-success '+disabled+'" data-tooltip="true" title="Acknowledge"><i class="fa fa-check"></i></a>&nbsp;';

		        actions +='</nobr>';
		        return actions;

	}
	function genericAckActionFormatter(destination){
		return function (value,row) {
			var actions = '<nobr>';
			var dest = destination;
	            // Add some overrides for any funny urls we have

	        actions += '<a href="{{ url('/') }}/' + dest + '/' + row.id + '/acknowledge" class="btn btn-sm btn-success" data-tooltip="true" title="Acknowledge"><i class="fa fa-check"></i></a>&nbsp;';
    ...
    for (var i in formatters) {
        ...
        window[formatters[i] + 'AssignFormatter'] = genericAssignFormatter(formatters[i]);
        window[formatters[i] + 'AssignActionFormatter'] = genericAssignActionFormatter(formatters[i]);
        window[formatters[i] + 'SelfActionFormatter'] = genericSelfActionFormatter(formatters[i]);
        window[formatters[i] + 'LogsActionFormatter'] = genericLogsActionFormatter(formatters[i]);
        window[formatters[i] + 'AgentActionFormatter'] = genericAgentActionFormatter(formatters[i]);
        window[formatters[i] + 'AckActionFormatter'] = genericAckActionFormatter(formatters[i]);
    }
   ...
   function imageFormatter(value) {
        if (value) {
            return '<a href="' + value + '" data-toggle="lightbox" data-type="image"><img src="' + value + '" style="max-height: {{ $SuhasSettings->thumbnail_max_h }}px; width: auto;" class="img-responsive"></a>';
        }
    }

    function fileUploadFormatter(value) {
        if ((value) && (value.url) && (value.inlineable)) {
            return '<a href="' + value.url + '" data-toggle="lightbox" data-type="image"><img src="' + value.url + '" style="max-height: {{ $SuhasSettings->thumbnail_max_h }}px; width: auto;" class="img-responsive"></a>';
    ...
    $(function () {
        $('#bulkEdit').click(function () {
            var selectedIds = $('.Suhas-table').bootstrapTable('getSelections');
