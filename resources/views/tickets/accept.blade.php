...
@section('content')
    <div class="row">
        <!-- left column -->
        <div class="col-md-7">
            <form class="form-horizontal" method="post" action="{{ route('tickets.accept.post', $ticket->id) }}" autocomplete="off">
                {{csrf_field()}}
           	...
           	<div class="form-group {{ $errors->has('comment') ? 'error' : '' }}">
   				 <label for="comment" class="col-md-3 control-label">Comments</label>
                        ...
                        <div class="box-footer">
                            ...
                            <button type="submit" class="btn btn-success pull-right"><i class="fa fa-check icon-white"></i>Accept</button>
                        </div>
           </div> <!-- /.box-->
            </form>
