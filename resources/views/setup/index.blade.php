  ...
  <tbody>
    <tr {!! ($start_settings['url_valid']) ? ' class="success"' : ' class="danger"' !!}>
      ...
      <td>
        ...
        @else
          Uh oh! Asset-IT thinks your URL is {{ $start_settings['url_config'] }}, but your real URL is {{ $start_settings['real_url'] }}
    ...
    <tr {!! ($start_settings['prod']) ? ' class="success"' : ' class="warning"' !!}>
      ...
      <td>
        ...
        @else
          Your app is set <code>{{ $start_settings['env'] }}</code> instead of <code>production</code> mode. If you're not planning on developing on Asset-IT, please update your <code>APP_ENV</code> settings in your  <code>.env</code> file to <code>production</code>.
        @endif
      </td>
    </tr>
