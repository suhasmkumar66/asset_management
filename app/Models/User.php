...
class User extends AssetModel implements AuthenticatableContract, CanResetPasswordContract
{
    ...
    public function isDepartment(){
        $department =DB::table('users')->where('id',Auth::user()->id)->where('department_id',1)->get();
        foreach($department as $dpt)
        {
            if($dpt->department_id == 1) return true;
        }
        return false;
    ...
    public function isTicketClosed(){
        $ticket = DB::table('tickets')
                  ->where('user_id',Auth::user()->id)
                  ->get();
                  foreach ($ticket as $tkt){
                      if($tkt->status == "closed") return true;
                  }
                  return false;
    }
    
    public function  isSelfAssigned($id){
        $ticket = DB::table('ticket_assign')
        ->where('ticketId',$id)
        ->get();
        foreach($ticket as $tkt){
            if($tkt->assigned_to == 1 || $tkt->coordinator == 1 || $tkt->collaborator == 1) return true;
        }
        return false;
    }
