...
class CustomField extends Model
{
     ...
     * do with previously existing values. - @Asset
     *
    ...
    {
        return "_Assetit_" . preg_replace("/[^a-zA-Z0-9]/", "_", strtolower($name));
    }
    ...
    public function convertUnicodeDbSlug($original = null)
    {
        ...
        if (!function_exists('transliterator_transliterate')) {
            $long_slug = '_Assetit_' . str_slug(\Patchwork\Utf8::utf8_encode(trim($name)), '_');
        } else {
            $long_slug =  '_Assetit_' . Utf8Slugger::slugify($name, '_');
        }
