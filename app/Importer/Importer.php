...
abstract class Importer
{
    ...
    private $defaultFieldMap = [
        ...
        'company' => 'company',
        'hod_name' => 'hod name',
        'hod_empid' => 'hod emp id',
        'res_name' => 'responsible name',
        'res_empid' => 'responsible emp id',
        'end_username' => 'end username',
    ];
    /**
    ...
    public function createOrFetchDepartmentFrAsset($department,$building,$floor,$wing,$room_name)
    {
        if ($department!='') {
            ...
            if($departmentcheck){
                ...
                if($wing!= ''){
                    ...
                    if($wingchek){
                        $this->log('A matching Wing ' . $wing . ' to department' . $department . ' already exists');
                    }
                    else{
                        $wingcreate = new Wing();
                        ...
                        if($wingcreate->save()){
                            $this->log('Wing ' . $wing . ' to department ' . $department . '  was created');
                        }
                    }
                }
                if($room_name!= ''){
                    ...
                    if($roomchek){
                        $this->log('A matching room ' . $room_name . ' to department' . $department . ' already exists');
                    }
                    else{
                        $roomcreate = new Room();
                        ...
                        if($roomcreate->save()){
                            $this->log('Room ' . $room_name . ' to department ' . $department . '  was created');
                        }
                    }
                }
                return $dept_id;
                ...
                if ($departmentcreate->save()) {
                         ...
                         $this->log('Department ' . $department . ' was created');
    ...
    public function createOrFetchHODFrAsset($empname,$empId,$dept_id){
        if($empId != ''){
            $usercheck = User::where('employee_num', '=', $empId)->first();
            if($usercheck){
                $user_id = $usercheck->id;
                $this->log('User '.$user_id.' already exists');
                return $user_id;
            }
            else{
                $usercreate = new User();
                $usercreate->employee_num = $empId;
                $usercreate->first_name = $empname;
                $usercreate->username = $empId;
                $usercreate->department_id = $dept_id;
                $usercreate->password = $this->tempPassword;
                if($usercreate->save()){
                    $user_id = $usercreate->id;
                    $this->log('User '.$empname.' was created');
                    return $user_id;
                }
            }
        }
